...
route.post('/register', async (req, res) => {
  ...
  const searchResult = await users.findOne({ $or: [{"username": user.username}, {"email": user.email}] });
  if (searchResult) {
...
route.patch('/:id', async (req, res) => {
  ...
  if (!params) return res.status(400).json({ message: 'User data is required' });
  const users = client.db(dbName).collection('users');
  ...
  if (result.modifiedCount == 1) {
    ...
    await createLog(user._id, `User ${printUsername} updated profile, changed ${Object.keys(params)}`);
