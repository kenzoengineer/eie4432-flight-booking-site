...
route.get('/:id', async (req, res, next) => {
  ...
  const seatResults = await seats.aggregate([
    {
      $match: {
...
route.patch('/:id', async (req, res, next) => {
  ...
  const newFlight = req.body;
  //update flight
  const result = await flights.findOneAndUpdate({ _id: new ObjectId(req.params.id) }, { $set: newFlight }, { returnDocument: 'after'});
  if (!result) {
  ...
  const seats = client.db(dbName).collection('seats');
  const seatOwners = await seats.find({ flight_id: new ObjectId(req.params.id), occupied: { $ne: null } }, {projection: { occupied: 1 }} ).toArray();
  const seatOwnerId = seatOwners.map(seat => seat.occupied);

  const users = client.db(dbName).collection('users');
  const flightDate = new Date(result.date).toDateString();
  const userResult = await users.updateMany({ _id: { $in: seatOwnerId } }, { $push: { messages: `Flight to ${result.dest} on ${flightDate} has been modified` } });
...
route.delete('/:id', async (req, res, next) => {
  ...
  const seats = client.db(dbName).collection('seats');
  const seatOwners = await seats.find({ flight_id: newFlight._id , occupied: { $ne: null } }, {projection:{ occupied: 1 }}).toArray();
  const seatOwnerId = seatOwners.map(seat => seat.occupied);

  const users = client.db(dbName).collection('users');
  const flightDate = new Date(newFlight.date).toDateString();

  const userResult = await users.updateMany({ _id: { $in: seatOwnerId } }, { $push: { messages: `Flight to ${newFlight.dest} on ${flightDate} has been cancelled` } });
  if (!userResult.acknowledged) {
    next(new Error('Unable to notify users'));
    return;
  }
